...
// protected:
    struct FrameSettings {
        ...
        byte *imageData;                        // Raw image data in VoSPI frame buffer, after 2B ID (unowned ptr, pitch of SPI frame line size)
        byte *telemetryData;                    // Raw telemetry data in VoSPI frame buffer, after 2B ID (unowned ptr, pitch of SPI frame line size)
    };
    ...
    byte _isrVSyncPin;                          // ISR vsync pin (Lepton FLiR Breakout v2+ only)
    SPISettings _spiSettings;                   // SPI port settings to use
    LeptonFLiR_CameraType _cameraType;          // Camera type in use
    ...
    byte *getImageData(int row, int section);   // Raw image data accessor (unowned ptr)

    byte *getTelemetryData(int row);            // Raw telemetry data accessor (unowned ptr)
